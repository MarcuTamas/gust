...
class _EditLogScreenState extends ConsumerState<EditLogScreen> {
  ...
  void _loadLog() {
    final sugarLogState = ref.read(sugarLogProvider);
    final log = sugarLogState.logs.firstWhere(
      (log) => log.id == widget.logId,
      orElse: () => sugarLogState.logs.isNotEmpty ? sugarLogState.logs.first : throw Exception('Log not found'),
    );

    if (log != null) {
      _productNameController.text = log.productName;
      _sugarGramsController.text = log.sugarGrams.toString();
      _contextNoteController.text = log.contextNote ?? '';
      _locationController.text = log.location ?? '';
      _selectedSugarType = log.sugarType;
      _selectedEmotion = Emotion.values.firstWhere(
        (e) => e.value == log.emotion.toUpperCase(),
        orElse: () => Emotion.neutral,
      );
      _wasCraving = log.wasCraving;
      _time = TimeOfDay(hour: log.hour, minute: log.minute);
    }
  }
        ...
        child: Form(
          ...
          child: Column(
            children: [
              ...
              LoadingButton(
                isLoading: sugarLogState.isUpdating,
  ...
  void _handleSubmit() async {
    if (_formKey.currentState!.validate()) {
      ...
      final success = await ref.read(sugarLogProvider.notifier).updateLog(widget.logId, request);
      if (success) {
        if (mounted) {
          context.pop();
        }
